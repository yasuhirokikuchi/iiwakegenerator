...
[https://iiwakegenerator.vercel.app/](https://iiwakegenerator.vercel.app/)
...
3. **開発サーバーを起動**
   ...
   ブラウザで **Vercel が表示する URL**（通常 `http://localhost:3000`）にアクセスしてください。

   > **注意**: ターミナルに `http://localhost:5173` と表示されても、それは Vite 単体の URL です。API を使うには `http://localhost:3000` 側を開いてください。`5173` だけ開くと API エラーになります。

   > 初回は Vercel CLI のログインを求められる場合があります。ポートが使用中の場合は、古い `vercel dev` プロセスを終了してから再起動してください。
...
### .......という感じで制作を進めていたが、APIの利用制限による学生には厳しい壁に当たってしまい無事没となりました
